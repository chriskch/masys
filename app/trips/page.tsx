...
const statusToColor: Record<Trip["status"], string> = {
  ...
  "In Planung": "bg-sky-500",
  Auswertung: "bg-amber-500",
};
...
export default function TripsPage() {
  ...
  const filteredTrips = useMemo(() => {
    ...
    return TRIPS.filter((trip) => {
      ...
      return matchesSearch && matchesBoat && matchesStart && matchesEnd;
    });
  }, [searchTerm, boat, dateRange]);
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card className="border-none !bg-white shadow-sm">
        ...
        <DataTable
          ...
          stripedRows
          size="small"
          emptyMessage="Keine Fahrten gefunden."
        >
          <Column
            header="Fahrt"
            body={(trip: Trip) => (
          ...
          <Column
            ...
            body={(trip: Trip) => (
              <span
                className={`trip-status-dot inline-flex h-3 w-3 rounded-full ${statusToColor[trip.status]} cursor-help`}
          ...
          <Column
            ...
            body={(trip: Trip) => (
              <Link
                href={`/trips/${trip.id}`}
                className="inline-flex items-center gap-2 rounded-full border border-sky-100 px-3 py-1 text-sm font-medium text-sky-600 hover:bg-sky-50 hover:text-sky-700"
              >
                Details
